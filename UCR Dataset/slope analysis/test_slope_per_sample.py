...
slope_40 = np.abs(recent_slope_before_cutoff(X_test_norm, 38, window=2))
slope_30 = np.abs(recent_slope_before_cutoff(X_test_norm, 30, window=2))
slope_50 = np.abs(recent_slope_before_cutoff(X_test_norm, 50, window=2))
...
print(f"T=41 | median: {np.median(slope_40):.4f} | 25th pct: {np.percentile(slope_40,25):.4f} | 75th pct: {np.percentile(slope_40,75):.4f}")
...
print(f"T=30 vs T=41: statistic={stat_30_40:.1f}, p-value={p_30_40:.6f}")
print(f"T=50 vs T=41: statistic={stat_50_40:.1f}, p-value={p_50_40:.6f}")
...
print(f"\nFraction of samples where T=41 slope < T=30 slope: {frac_lower_than_30:.3f}")
print(f"Fraction of samples where T=41 slope < T=50 slope: {frac_lower_than_50:.3f}")
